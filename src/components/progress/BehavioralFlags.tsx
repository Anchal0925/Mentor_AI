...
export function BehavioralFlags() {
  ...
  const apiFlags = dashboard?.behavioral_flags ?? [];

  // Map API flags → display format
  const flags = apiFlags.length > 0
    ? apiFlags.map((f) => ({
        id: f.id,
        type: f.severity === 'low' ? 'positive' : 'negative',
        title: f.flag.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()),
        desc: `Severity: ${f.severity}`,
        occurrences: 1,
      }))
    : [
        // fallback static data when no API data yet
  ...
  return (
    <Card className="col-span-1 h-[400px] flex flex-col overflow-hidden">
      ...
      <p className="font-mono text-[10px] text-muted">{flags.length} patterns tracked</p>
          ...
          return (
            <motion.div
              ...
              className={`p-3 rounded-lg border flex items-start gap-3 ${
                isNeg ? 'bg-accent-red/5 border-accent-red/20' : 'bg-accent-green-bg border-accent-green/20'
              }`}
            >
              {isNeg
